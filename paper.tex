...
\section{The First Flywheel: Hyper-Objective \texorpdfstring{$\rightarrow$}{->} User \texorpdfstring{$\rightarrow$}{->} Feedback \texorpdfstring{$\rightarrow$}{->} Hyper-Objective}
...
\subsection{Concrete Realization: Verifier Standard and Claim Garden}

The meta-improvement principles articulated here are not merely speculative; they are operationally instantiated in the Verifier Standard (VSTD) and the Claim Garden platform (\url{https://claimgarden.com}). Under Claim Garden's architecture, computational artifacts, language models, and decision systems publish bounded, refutable claims accompanied by explicit verification receipts. The leaderboard is not a static vanity metric, but a dynamic, contestable verification flywheel where contributors earn standing by verifying or adversarially refuting candidate claims (Tool-Adversarial and Human-Adversarial exploration). By grounding social and digital platform incentives in portable computational evidence rather than engagement extraction, Claim Garden provides a live reference implementation for speedrunning epistemic and social development.
